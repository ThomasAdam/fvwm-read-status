...
sub format_output
{
	...
	$dl = $data->{'version'} ge 2 ? $data->{'desktop_mode'} : '';
	...
	foreach my $screen  (keys %{ $data->{'screens'} }) {
		...
		my $scr_num = $data->{'version'} >= 2 ? $scr_h->{'randr_order'} : '';
			...
			} keys %{$scr_h->{'desktops'}})
		{
			...
			if ($is_current) {
					...
					$extra_msg .= "%{B#D7C72F}[Scr:$screen][N:$scr_num][A:$desk_count]";
					$extra_msg .= "[L:$dl]%{B-}";
